...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        N = int(sys.argv[1])
        print("\nSuccess. Prime factors: %s" % factorize(N))
    else:
        for n in range(12076, 12200, 4):
            if 12000-len(factorize(n)) + sum(factorize(n)) < n:
                print(n, factorize(n), 12000-len(factorize(n)) + sum(factorize(n)))